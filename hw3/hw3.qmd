...
# Read info from diagnoses_icd.csv.gz, d_icd_procedures.csv.gz, d_icd_diagnoses.csv.gz
...
icustays <- fread(file_path)
icustays <- icustays[subject_id == id]
...
# icustays_vitals <- icustays_info |>
#   merge(vitals, by = c("subject_id", "stay_id"))
```


```{r}
compute_breaks <- function(data) {
  # Find the range and set appropriate breaks
  min_value <- min(data$value, na.rm = TRUE)
  max_value <- max(data$value, na.rm = TRUE)
  range_value <- max_value - min_value
  # Adjust step size based on the range of values
  step_size = ceiling(range_value / (max_value - min_value) * 5)
  return(seq(min_value, max_value, by = step_size))
}
```
