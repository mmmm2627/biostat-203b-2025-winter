...
Read info from `patients.csv.gz` with `subject_id = 10063848`:
```{r}
file_path <- '~/mimic/hosp/patients.csv.gz'

patient <- fread(file_path)
patient <- patient[subject_id == 10063848]
patient
```
...
admissions <- admissions[subject_id == 10063848]
admissions
```
...
transfers <- transfers[subject_id == 10063848]
transfers
```
