...
The extension `gz` means that the file is compressed. The data files have large sizes, so they have to be zipped to be dowloaded faster.
...
Note: `uniq` detects duplicates only if they are next to each other, so need to `sort` first.
